...
export default function YieldSphere() {
  ...
  const UPDATE_INTERVAL = 50; // Update UI every 50ms
   ...
   // Initialize settings with optimal default values for MAXI
   const [settings] = useState<VisualizationSettings>({
    // Sphere Configuration
    radius: 3,
    spiralPoints: 5555, // MAXI stake length
    spiralTurns: 1000,
    ...
    // Visual Settings
    baseColorHex: '#3991ED', // Blue color for MAXI
    inactiveOpacity: 0.1,
  ...
  }, [settings, yieldData, isLoading, availableDays, hasEntered]);
    ...
    <div className="w-full h-screen overflow-hidden cursor-grab active:cursor-grabbing" ref={canvasRef}>
      ...
      <div className="absolute top-4 left-4 text-white z-10 bg-black/20 backdrop-blur-sm p-4 rounded-xl border-2 border-white/10">
        <div className="flex items-center gap-2">
          ...
          />
          <h1 className="text-2xl font-bold">MAXI</h1>
        </div>
