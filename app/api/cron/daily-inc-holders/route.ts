...
const INC_CONTRACT = '0x2fa878ab3f87cc1c9737fc071108f904c0b0c95d';
const INC_DECIMALS = 18;
const JOB_NAME = 'inc-holders-collection';
...
async function getOrCreateProgress(supabase: any, date: string): Promise<ProgressState> {
  ...
  if (existing && !existing.is_complete) {
    ...
    const nextPageParams = existing.last_address_hash ? {
      ...
      items_count: 50,
      ...(existing.last_value ? { value: existing.last_value } : {}) // Include value if it exists
    } : null;
...
async function updateProgress(supabase: any, date: string, progress: Partial<ProgressState>, lastItem?: any) {
  ...
  if (lastItem) {
    ...
    updateData.last_value = lastItem.value; // Store value directly as text field can handle large values
  }
...
async function fetchIncHoldersFromPage(
  startPage: number, 
  nextPageParams: any, 
  maxPages: number = 200
): Promise<{ holders: IncHolder[], totalPages: number, isComplete: boolean }> {
  const baseUrl = `https://api.scan.pulsechain.com/api/v2/tokens/${INC_CONTRACT}/holders`;
  const holders: IncHolder[] = [];
  ...
  while (hasNextPage && pageCount < startPage + maxPages) {
    try {
      ...
      const newHolders = data.items.map((item: any) => ({
        ...
        balance: Number(item.value) / Math.pow(10, INC_DECIMALS),
...
async function getTotalSupply(supabase: any): Promise<number> {
  try {
    console.log('📊 Fetching INC total supply from database...');
    ...
    if (error) {
      console.error('Error fetching INC supply from database:', error);
      throw error;
    }

    if (!data) {
      throw new Error('No INC supply data found in database');
    }

    const totalSupply = parseFloat(data.total_supply_formatted);
    console.log('📊 INC Total Supply from DB:', totalSupply.toLocaleString());
    return totalSupply;
  } catch (error) {
    console.error('Error fetching INC supply:', error);
    throw error;
  }
}
...
async function getAllHolders(supabase: any): Promise<IncHolder[]> {
  const today = new Date().toISOString().split('T')[0];
  
  console.log('📊 Fetching all INC holders from database...');
  ...
  console.log(`📊 Total INC holders in database: ${count}`);
  
  // Supabase has a default limit of 1000, so we need to paginate
  const allHolders: IncHolder[] = [];
  const pageSize = 1000;
  let from = 0;
...
async function clearLeagueTable(supabase: any): Promise<void> {
  ...
  console.log('🗑️ Clearing league_inc table for fresh start...');
  ...
  try {
    ...
    if (error) {
      console.error('Error clearing league_inc table:', error);
      throw error;
    }

    console.log('✅ league_inc table cleared successfully');
  } catch (error) {
...
async function getTotalHolders() {
  try {
    ...
    const url = `https://api.scan.pulsechain.com/api/v2/tokens/${INC_CONTRACT}/counters`;
    ...
    console.log(`📊 Total INC holders from API: ${totalHolders.toLocaleString()}`);
    return totalHolders;
  } catch (error) {
...
async function calculateLeagueStatistics(holders: IncHolder[], totalSupply: number): Promise<Record<string, LeagueStats>> {
  console.log('📊 Calculating INC league statistics...');
  console.log(`📊 Total INC Supply: ${totalSupply.toLocaleString()}`);
...
async function calculateLeagues(supabase: any): Promise<any> {
  console.log('\n🏆 Starting INC league calculation...');
  ...
  try {
    ...
    console.log('\n📈 INC LEAGUE CALCULATION SUMMARY');
    console.log('=====================================');
    console.log(`💰 Total INC Supply: ${totalSupply.toLocaleString()}`);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log('🚀 Starting INC holders collection...');
    ...
    if (progress.lastPage === 0 && progress.totalCollected === 0) {
      ...
      if (clearBackupError) {
        ...
        console.log('✅ Cleared inc_holders_last_week table');
      }
      ...
      console.log('🗑️ Clearing inc_holders table...');
    ...
    const result = await fetchIncHoldersFromPage(
      progress.lastPage, 
    ...
    if (result.holders.length > 0) {
      ...
      if (newTotal >= MAX_HOLDERS_TO_COLLECT) {
        ...
        return NextResponse.json({
          ...
          leagueCalculationTriggered: false
        });
      }
      ...
      if (currentTotal >= MAX_HOLDERS_TO_COLLECT) {
        ...
        setTimeout(async () => {
          try {
            ...
            const response = await fetch(`${baseUrl}/api/cron/daily-inc-holders`, {
              method: 'GET',
              headers: {
    ...
    console.error('❌ Error in INC holders collection:', error);
