...
const ICSA_CONTRACT = '0xfc4913214444af5c715cc9f7b52655e788a569ed';
const ICSA_DECIMALS = 9;
const JOB_NAME = 'icsa-holders-collection';
...
// Constants
const MAX_HOLDERS_TO_COLLECT = 10000;
...
async function getOrCreateProgress(supabase: any, date: string): Promise<ProgressState> {
  ...
  if (existing && !existing.is_complete) {
    ...
    const nextPageParams = existing.last_address_hash ? {
      ...
      items_count: 50,
      value: existing.last_value?.toString() // Include value for proper pagination
    } : null;
...
async function updateProgress(supabase: any, date: string, progress: Partial<ProgressState>, lastItem?: any) {
  ...
  if (lastItem) {
    ...
    updateData.last_value = lastItem.rawValue; // Store raw value for pagination
  }
...
async function fetchIcsaHoldersFromPage(
  startPage: number, 
  nextPageParams: any, 
  maxPages: number = 200
): Promise<{ holders: IcsaHolder[], totalPages: number, isComplete: boolean }> {
  const baseUrl = `https://api.scan.pulsechain.com/api/v2/tokens/${ICSA_CONTRACT}/holders`;
  const holders: IcsaHolder[] = [];
  ...
  while (hasNextPage && pageCount < startPage + maxPages) {
    try {
      ...
      const newHolders = data.items.map((item: any) => ({
        ...
        balance: Number(item.value) / Math.pow(10, ICSA_DECIMALS),
...
async function getTotalSupply(supabase: any): Promise<number> {
  try {
    console.log('📊 Fetching ICSA total supply from database...');
    ...
    if (error) {
      console.error('Error fetching ICSA supply from database:', error);
      throw error;
    }

    if (!data) {
      throw new Error('No ICSA supply data found in database');
    }

    const totalSupply = parseFloat(data.total_supply_formatted);
    console.log('📊 ICSA Total Supply from DB:', totalSupply.toLocaleString());
    return totalSupply;
  } catch (error) {
    console.error('Error fetching ICSA supply:', error);
    throw error;
  }
}
...
async function getAllHolders(supabase: any): Promise<IcsaHolder[]> {
  const today = new Date().toISOString().split('T')[0];
  
  console.log('📊 Fetching all ICSA holders from database...');
  ...
  console.log(`📊 Total ICSA holders in database: ${count}`);
  
  // Supabase has a default limit of 1000, so we need to paginate
  const allHolders: IcsaHolder[] = [];
  const pageSize = 1000;
  let from = 0;
...
async function clearLeagueTable(supabase: any): Promise<void> {
  ...
  console.log('🗑️ Clearing league_icsa table for fresh start...');
  ...
  try {
    ...
    if (error) {
      console.error('Error clearing league_icsa table:', error);
      throw error;
    }

    console.log('✅ league_icsa table cleared successfully');
  } catch (error) {
...
async function getTotalHolders() {
  try {
    ...
    const url = `https://api.scan.pulsechain.com/api/v2/tokens/${ICSA_CONTRACT}/counters`;
    ...
    console.log(`📊 Total ICSA holders from API: ${totalHolders.toLocaleString()}`);
    return totalHolders;
  } catch (error) {
...
async function calculateLeagueStatistics(holders: IcsaHolder[], totalSupply: number): Promise<Record<string, LeagueStats>> {
  console.log('📊 Calculating ICSA league statistics...');
  console.log(`📊 Total ICSA Supply: ${totalSupply.toLocaleString()}`);
...
async function calculateLeagues(supabase: any): Promise<any> {
  console.log('\n🏆 Starting ICSA league calculation...');
  ...
  try {
    ...
    console.log('\n📈 ICSA LEAGUE CALCULATION SUMMARY');
    console.log('=====================================');
    console.log(`💰 Total ICSA Supply: ${totalSupply.toLocaleString()}`);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log('🚀 Starting ICSA holders collection...');
    ...
    if (progress.lastPage === 0 && progress.totalCollected === 0) {
      ...
      if (clearBackupError) {
        ...
        console.log('✅ Cleared icsa_holders_last_week table');
      }
      ...
      console.log('🗑️ Clearing icsa_holders table...');
    ...
    const result = await fetchIcsaHoldersFromPage(
      progress.lastPage, 
    ...
    if (result.holders.length > 0) {
      ...
      await supabase
        ...
        .update({ 
          ...
          last_value: lastItem.rawValue, // Use correct field name and raw value
      ...
      if (currentTotal >= MAX_HOLDERS_TO_COLLECT) {
        ...
        setTimeout(async () => {
          try {
            ...
            const response = await fetch(`${baseUrl}/api/cron/daily-icsa-holders`, {
              method: 'GET',
              headers: {
    ...
    console.error('❌ Error in ICSA holders collection:', error);
