    ...
    const fetchData = async () => {
      try {
        ...
        const { data: response, error } = await supabase
          .from(tableName)
          .select('date, hex_price, maxi_price, deci_price, lucky_price, trio_price, base_price')
        ...
        const formattedData = response.map((item) => ({
          ...
          pHEX: item.hex_price ? parseFloat(item.hex_price) : null,
          pMAXI: item.maxi_price ? parseFloat(item.maxi_price) : null,
          pDECI: item.deci_price ? parseFloat(item.deci_price) : null,
          pLUCKY: item.lucky_price ? parseFloat(item.lucky_price) : null,
          pTRIO: item.trio_price ? parseFloat(item.trio_price) : null,
          pBASE: item.base_price ? parseFloat(item.base_price) : null
        }));
