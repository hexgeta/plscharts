...
import { useTokenPrices, TokenPriceData } from '@/hooks/crypto/useTokenPrices'
...
function TokenCard({ symbol, name, logo, priceData, isLoading }: { 
  symbol: string
  name: string
  logo: string
  ...
  return (
    <Card className="bg-black/20 backdrop-blur-sm text-white p-4 rounded-xl border-2 border-white/10 flex flex-col gap-2 min-w-[260px]">
      ...
      <div className="flex items-baseline gap-2">
        ...
        <div className={`text-xs font-bold ${getPriceChangeColor(priceData?.priceChange.h24 ?? 0)}`}>
          {isLoading || !priceData ? '' : formatNumber(priceData.priceChange.h24, { decimals: 1, percentage: true, alreadyPercentage: true })}
        </div>
      </div>
      <div className="flex flex-wrap gap-4 mt-2 text-sm">
        <div>
          ...
          <div className="font-semibold">
            {isLoading || !priceData?.marketCap ? '--' : `$${formatNumber(priceData.marketCap, { decimals: 1, compact: true })}`}
          </div>
        </div>
        <div>
          <div className="text-zinc-500">Supply</div>
          <div className="font-semibold">
            {isLoading || !priceData?.supply ? '--' : formatNumber(priceData.supply, { decimals: 1, compact: true })}
          </div>
        </div>
        <div>
          <div className="text-zinc-500">Volume</div>
          <div className="font-semibold">
            {isLoading || !priceData?.volume.h24 ? '--' : `$${formatNumber(priceData.volume.h24, { decimals: 1, compact: true })}`}
          </div>
        </div>
      </div>
    </Card>
...
export default function PulseDashboard() {
  const { prices, isLoading } = useTokenPrices(TOKENS.map(t => t.symbol))
