...
const SINE_AMPLITUDE = 0.8;     // Range: 0.1 to 2 - Controls wave height
...
function calculateExponentialRegression(data: ProjectionDataPoint[], curveIntensity = EXPONENTIAL_CURVE_INTENSITY) {
  ...
  data.forEach(point => {
    if (point.backingRatio) {
      const x = point.day - startX;
      const lnY = Math.log(point.backingRatio);
...
function ExtendedProjectionChart({ title = 'pMAXI Ⓜ️' }: ProjectionChartV2Props) {
  ...
  const projectionData = useMemo(() => {
    ...
    const regression = calculateExponentialRegression(rawData, EXPONENTIAL_CURVE_INTENSITY);
    const linearRegression = calculateLinearRegression(rawData, LINEAR_SLOPE_MULTIPLIER);
    const sineRegression = calculateSineRegression(rawData, regression);

    // Get the last day from historical data
    const lastDay = rawData[rawData.length - 1].day;
    ...
    for (let i = 0; i <= 100; i++) {
      ...
      projectionPoints.push({
        day,
        date: new Date(new Date('2022-05-01').getTime() + (day - 881) * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        backingRatio: regression.calculate(day),
    ...
    return [...rawData, ...projectionPoints];
  }, [rawData]);
