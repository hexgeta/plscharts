...
export function LoginTable() {
  ...
  const { prices, isLoading: pricesLoading } = useTokenPrices(ALL_TOKENS);
  ...
  useEffect(() => {
    ...
    const allBackingLoaded = Object.values(isEthereum ? ethBackingMap : pulseBackingMap)
      .every(({ isLoading }) => !isLoading);
    
    if (!pricesLoading && allBackingLoaded) {
  ...
  }, [pricesLoading, ethBackingMap, pulseBackingMap, isEthereum, isAnimating]);
    ...
    <AnimatePresence mode="wait">
      {isLoading ? (
        ...
        <motion.div
          ...
          className="w-full max-w-6xl mx-auto rounded-xl p-4 bg-black/5 backdrop-blur-sm border-2 border-white/10 h-auto relative"
        >
              ...
              <table className="w-full">
                ...
                <tbody>
                  {tokens.map((token) => {
                    ...
                    const { backingData } = backingDataMap[token];
                    const dexscreenerUrl = getDexscreenerUrl(token);
