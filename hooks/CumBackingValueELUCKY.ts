...
if (!TOKEN_CONSTANTS?.eLUCKY?.LAUNCH_DATE) {
  throw new Error('eLUCKY launch date is not defined in TOKEN_CONSTANTS');
}

if (!TOKEN_CONSTANTS?.eLUCKY?.TSHARES) {
  throw new Error('eLUCKY T-shares are not defined in TOKEN_CONSTANTS');
}

if (!TOKEN_CONSTANTS?.eLUCKY?.STAKE_PRINCIPLE) {
  throw new Error('eLUCKY stake principle is not defined in TOKEN_CONSTANTS');
}

if (!TOKEN_CONSTANTS?.eLUCKY?.TOKEN_SUPPLY) {
  throw new Error('eLUCKY token supply is not defined in TOKEN_CONSTANTS');
}
...
export const CumBackingValueELUCKY = () => {
  ...
  const { data: priceData, error: priceError } = useSWR(
    PRICE_CACHE_KEYS.daily('elucky'),
    async () => {
      console.log('Fetching eLUCKY price data...');
      const startDate = TOKEN_CONSTANTS.eLUCKY.LAUNCH_DATE!.toISOString().split('T')[0];
  ...
  const processedData = useMemo(() => {
    ...
    const START_DATE = TOKEN_CONSTANTS.eLUCKY.LAUNCH_DATE!;
    const TOKEN_SUPPLY = TOKEN_CONSTANTS.eLUCKY.TOKEN_SUPPLY!;
